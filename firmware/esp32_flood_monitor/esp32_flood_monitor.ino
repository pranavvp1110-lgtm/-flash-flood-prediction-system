 ...
 *  --------------------------------------------------------------------------------------
 *  FOR ESP32-S3:
 *    - DHT11 Temp & Humidity      : VCC -> 3.3V | GND -> GND | DATA/OUT -> GPIO
 * 4 (IO4)
 *    - MPU-6050 (GY-521) 6-Axis   : VCC -> 5V/VIN or 3.3V | GND -> GND | SDA ->
 * GPIO 8 | SCL -> GPIO 9 | AD0 -> GND
 *    - Soil Moisture Sensor       : VCC -> 3.3V | GND -> GND | AOUT -> GPIO 1
 * (IO1 - ADC1)
 *    - HW-038 Raindrop Sensor     : VCC -> 3.3V | GND -> GND | A0/AOUT -> GPIO
 * 2 (IO2 - ADC1) | D0 -> GPIO 3 (IO3)
 *    - Piezo Buzzer               : (+) -> GPIO 5 (IO5) | (-) -> GND
 *    - Status LEDs                : Green -> GPIO 6 (IO6) | Yellow -> GPIO 7
 * (IO7) | Red -> GPIO 10 (IO10)
 *
 *  FOR CLASSIC ESP32 (DevKit V1):
 *    - DHT11 Temp & Humidity      : VCC -> 3.3V | GND -> GND | DATA/OUT -> GPIO
 * 4
 *    - MPU-6050 (GY-521) 6-Axis   : VCC -> 5V/VIN or 3.3V | GND -> GND | SDA ->
 * GPIO 21 | SCL -> GPIO 22 | AD0 -> GND
 *    - Soil Moisture Sensor       : VCC -> 3.3V | GND -> GND | AOUT -> GPIO 34
 * (ADC1)
 *    - HW-038 Raindrop Sensor     : VCC -> 3.3V | GND -> GND | A0/AOUT -> GPIO
 * 35 (ADC1) | D0 -> GPIO 18
 *    - Piezo Buzzer               : (+) -> GPIO 5 | (-) -> GND
 *    - Status LEDs                : Green -> GPIO 19 | Yellow -> GPIO 23 | Red
 * -> GPIO 25
 * ======================================================================================
 */
...
// Backend Telemetry Endpoint (Your computer's active local IP address on 'Hunters' Wi-Fi)
...
// #define BOARD_ESP32_CLASSIC  // Enable if using Standard ESP32 (ESP-WROOM-32
// / DevKit V1)
...
bool wakeAndConfigureMPU(uint8_t addr) {
  ...
  Wire.write(0x01); // PLL with X-Gyro reference (much more stable than internal
                    // 8MHz RC oscillator)
  if (Wire.endTransmission() != 0)
    return false;
  delay(20);
...
bool scanAndInitMPU() {
  ...
  // Attempt to initialize found MPU address or check both standard 0x68 and
  // 0x69
  ...
  for (int i = 0; i < 2; i++) {
    ...
    if (wakeAndConfigureMPU(addr)) {
      ...
      mpuOnline = true;
      Serial.printf("[I2C OK] MPU-6050 Motion Sensor successfully initialized "
                    "at 0x%02X!\n",
                    addr);
      return true;
    }
  ...
  mpuActiveAddress = 0;
  Serial.println(
      "[WARN] MPU-6050 found on bus but failed configuration. Retrying...");
  return false;
}
...
bool readMPURaw(float &pitchDeg, float &vibrationMag) {
  ...
  // Convert raw values for ±4g range: sensitivity is 8192 LSB/g (1g = 9.80665
  // m/s^2)
  ...
  // Calculate vibration magnitude (deviation from 1G gravity + rotational
  // speed)
...
void setupHardware() {
  ...
  Serial.printf(
      "[INIT] Initializing I2C Bus on SDA=GPIO%d, SCL=GPIO%d (100kHz)...\n",
      PIN_MPU_SDA, PIN_MPU_SCL);
...
void printWiFiDiagnostics() {
  ...
  case WL_NO_SSID_AVAIL:
    Serial.println("[WIFI ERROR] SSID not found! Ensure WiFi network is 2.4 "
                   "GHz (ESP32 cannot connect to 5 GHz).");
    break;
...
void loop() {
  ...
  if (currentMillis - lastTelemetryTime >= TELEMETRY_INTERVAL_MS) {
    ...
    int rainDigitalTrigger =
        digitalRead(PIN_RAIN_DIGITAL); // LOW when wet, HIGH when dry
    ...
    float deltaAccumMm =
        (rainRateMmH / 3600.0f) * (TELEMETRY_INTERVAL_MS / 1000.0f);
    ...
    Serial.printf("  🌱 Soil Moisture   : %.1f%%  [Raw ADC: %d]\n", soilPct,
                  soilRaw);
    Serial.printf("  🌧️ Rain (HW-038)   : %.1f mm/h (%.1f%%)  [Raw ADC: %d | "
                  "Digital: %s]\n",
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (httpResponseCode > 0) {
        ...
        if (!err) {
          ...
          Serial.printf(
              ">> AI PREDICTION: %s (Level %d) | Confidence: %.1f%%\n", riskCat,
              predictedRisk, conf);
